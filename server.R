...
shinyServer(function(input, output) {
  ...
  dataTables <- reactiveValues(log2cpm = NULL,
                                           tsne.data=NULL,
                                           featuredata=NULL,
                                           selectedDge=NULL)
  n_fun <- function(x){
  ...
  data<-reactive({
    ...
    featuredata<-featuredata[which(featuredata$Chromosome.Name %in% c(unlist(lapply(seq(1,22,1),toString)),c("X","Y","MT"))),]
  ...
  output$clusters1 <- renderUI({
    noOfClusters <- max(dataTables$tsne.data$dbCluster)
    selectInput("clusters1",label="Cluster", choices=c(0:noOfClusters),selected=1)
  })
  
  output$clusters2 <- renderUI({
    noOfClusters <- max(dataTables$tsne.data$dbCluster)
    selectInput("clusters2",label="Cluster", choices=c(0:noOfClusters),selected=1)
  })
  ...
  output$gene_vio_plot<-renderPlot({
    ...
    isolate({
      ...
      p1<-ggplot(tsne.data,aes(factor(dbCluster),values,fill=factor(dbCluster)))+
  ...
  output$downloadExpression <- downloadHandler(
    ...
    content = function(file) {
      ...
      cells<-rownames(subsetData[which(cells.names$selected_==TRUE),])
      #cat(file=stderr(),cells[1:5])
      
      if (length(cells)==1){
        subsetExpression<-dataTables$log2cpm[,cells]
        subsetExpression<-as.data.frame(subsetExpression,row.names=rownames(dataTables$log2cpm))
        colnames(subsetExpression)<-cells
        subsetExpression$Associated.Gene.Name<-dataTables$featuredata[rownames(subsetExpression),'Associated.Gene.Name']
  ...
  output$heatmap<-renderPlot({
    ...
    isolate({
      ...
      h<-pheatmap(as.matrix(expression),cluster_rows=TRUE,cluster_cols=FALSE,
                  scale='row',fontsize_row=10,labels_col = colnames(expression),
                  labels_row = dataTables$featuredata[rownames(expression),'Associated.Gene.Name'],
                  show_rownames = TRUE,annotation_col = annotation,
                  show_colnames = FALSE,annotation_legend = TRUE,
                  breaks= seq(-6, 6, by = .12),
                  colorRampPalette(rev(brewer.pal(n = 6, name =
                                                    "RdBu")))(100)
  ...
  output$clusterPlot2<-renderPlot({
    ...
    if (vvvvv$doPlot == FALSE) return()
    
    isolate({
      
      geneid<-rownames(
        dataTables$featuredata[which(dataTables$featuredata$Associated.Gene.Name==toupper(input$gene_id_sch)),])[1]
      ...
      p1<-ggplot(subsetData,aes_string(x=input$dimension_x2,y=input$dimension_y2))+
        geom_point(aes_string(size=2,color='values'))+
        geom_point(shape = 1,size = 4,colour = "black")+
        theme_bw()+
        theme(axis.text.x=element_text(angle=90, size=12, vjust=0.5), 
              axis.text.y=element_text(size=10), strip.text.x = element_text(size=16), 
              strip.text.y = element_text(size=14), axis.title.x = element_text(face="bold", size=16),
              axis.title.y = element_text(face="bold", size=16),
              legend.position="none")+
        ggtitle(paste(toupper(input$gene_id_sch),input$clusters2,sep='-Cluster'))+
        scale_colour_gradient2(low='grey50', high="red")
      p1
    })
  })
  ...
  output$selectedHeatmap<-renderPlot({
    ...
    isolate({
      ...
      subsetData<-subset(dataTables$tsne.data,dbCluster==input$clusters2)
      ...
      h<-pheatmap(as.matrix(expression),cluster_rows=TRUE,cluster_cols=TRUE,
                  scale='row',fontsize_row=10,labels_col = colnames(expression),
                  labels_row = dataTables$featuredata[rownames(expression),'Associated.Gene.Name'],
                  show_rownames = TRUE,
                  show_colnames = FALSE,
  ...
  output$dge_plot1<-renderPlot({
    ...
    isolate({
      ...
      p1<-ggplot(subsetData,aes_string(x=input$dimension_x1,y=input$dimension_y1))+
        geom_point()+
  ...
  dge<-reactive({
    ...
    isolate({
      ...
      toReturn<-DiffExpTest(subsetExpression,cells.1,cells.2,genes.use = genes.use)
      toReturn[,"avg_diff"]=total.diff[rownames(toReturn)]
      toReturn$Associated.Gene.Name<-dataTables$featuredata[rownames(toReturn),'Associated.Gene.Name']
  ...
  output$dge<-DT::renderDataTable({
    ...
    isolate({
      ...
      top.genes<-dge()
      top.genes$Associated.Gene.Name<-dataTables$featuredata[rownames(top.genes),'Associated.Gene.Name']
  ...
  output$download_dge_table <- downloadHandler(
    filename = function() { paste("SubCluster","DGE_table.csv",sep='_')},
    content = function(file) {
